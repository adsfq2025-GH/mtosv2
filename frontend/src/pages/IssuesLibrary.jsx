...
export default function IssuesLibrary() {
  const { user } = useAuth();
  ...
  const load = useCallback(async () => {
    ...
    setBusy(true);
    try {
      const res = await libraries.issues({ start, end, clientId, accountManagerId, q, limit: 1500 });
      setItems(res?.items || []);
    } catch (e) {
      setErr(e?.response?.data?.detail || e?.message || "Failed to load issues library");
      setItems([]);
    } finally {
      setBusy(false);
    }
  ...
  return (
    <div>
      <PageHead
        title="Issues Library"
        subtitle="Search and review issues across meetings. Defaults to the last 30 days."
        actions={
      ...
      <div className="card-flat p-5">
        <div className="grid grid-cols-1 md:grid-cols-5 gap-3">
          ...
          <div>
            ...
            <input className="input mt-1.5" value={q} onChange={(e) => setQ(e.target.value)} placeholder="Title, description, plan…" />
          </div>
        </div>
      ...
      <div className="mt-5 card-flat overflow-hidden">
        {visible.length === 0 && <div className="p-10 text-center text-slate-400">No issues found for this range.</div>}
        {visible.map((row, idx) => (
          <div key={`${row.meeting_id}-${idx}`} className={`p-4 ${idx !== visible.length - 1 ? "border-b border-white/5" : ""}`}>
            <div className="flex items-start justify-between gap-3">
              <div className="min-w-0">
                <div className="text-sm font-semibold truncate">{row.issue?.title || "Issue"}</div>
                ...
                {row.issue?.description && <div className="text-sm text-slate-200 mt-2">{row.issue.description}</div>}
                {row.issue?.action_plan && <div className="text-xs text-slate-400 mt-2">{row.issue.action_plan}</div>}
              </div>
